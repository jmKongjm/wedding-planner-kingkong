...
async function dbLoad(key) { try { const r = await fetch(SB_URL + "/rest/v1/couples?couple_key=eq." + encodeURIComponent(key) + "&select=data", { headers: H }); const rows = await r.json(); return rows?.[0]?.data || null; } catch { return null; } }
async function dbSave(key, data) { try { await fetch(SB_URL + "/rest/v1/couples", { method: "POST", headers: { ...H, "Prefer": "resolution=merge-duplicates,return=minimal" }, body: JSON.stringify({ couple_key: key, data, updated_at: new Date().toISOString() }) }); } catch (e) { console.error(e); } }
...
const TIPS={"2026-02/2026-04":["💡 웨딩홀 상담 시 견적서를 서면으로 꼭 받아오세요","💡 스드메는 3곳 이상 비교하면 가성비를 파악하기 좋아요","💡 예물 반지는 금값이 오르니 빨리 결정할수록 유리해요"],"2026-04/2026-05":["💡 드레스 투어 시 사진이 안 되는 곳이 많으니 메모를 준비하세요","💡 드레스 지정이 투어보다 20~40만원 저렴할 수 있어요"],"2026-06/2026-07":["💡 촬영가봉 때 악세서리도 함께 매칭해보세요","💡 신랑 예복 맞춤은 촬영 3개월 전까지 하셔야 해요"],"2026-09/2026-10":["💡 촬영 전날 충분히 수면하고, 야식은 피하세요","💡 촬영 당일 누드톤 속옷을 준비하세요"],"2026-11/2026-12":["💡 앨범 페이지 추가는 3.3만원/장이에요","💡 모바일 청첩장은 결혼 6~8주 전에 발송하세요"],"2026-11/2027-01":["💡 신혼집 입주와 결혼식을 맞춰 계획하면 좋아요","💡 폐백은 요즘 생략 추세, 양가 의견을 먼저 확인하세요"],"2027-01/2027-03":["💡 본식 가봉은 그날 바로 결정해야 해요","💡 식순표는 사회자와 공유, MR은 USB로 준비하세요"],"2027-04/2027-05":["💍 결혼 전날 편지를 써서 당일 교환해보세요","💍 비상 키트: 바늘, 테이프, 진통제, 여분 스타킹"]};
...
function Modal({title,open,onClose,children}){
  if(!open)return null;
  return(<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.35)",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={onClose}>
    <div style={{background:"#fff",borderRadius:24,padding:"32px 28px",maxWidth:520,width:"100%",boxShadow:"0 24px 64px rgba(0,0,0,0.15)",maxHeight:"90vh",overflowY:"auto",position:"relative"}} onClick={e=>e.stopPropagation()}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24}}>
        <h3 style={{fontSize:20,fontWeight:700,color:"#33691E",margin:0}}>{title}</h3>
        <button onClick={onClose} style={{background:"none",border:"none",fontSize:22,color:"#BCAAA4",cursor:"pointer",padding:"4px 8px"}}>✕</button>
      </div>
      {children}
    </div>
  </div>);
}
function ModalField({label,children}){return <div style={{marginBottom:18}}><label style={{fontSize:14,fontWeight:700,color:"#5D4037",display:"block",marginBottom:6}}>{label}</label>{children}</div>;}
...
function LoginScreen({onLogin}){
  ...
  const createNew=async()=>{if(!key.trim())return;setBusy(true);const d={...freshData(),isSetup:false};await dbSave(key.trim(),d);setBusy(false);onLogin(key.trim(),d);};
  return(<div style={S.wrap}><Fl1 s={{width:150,top:-30,left:-30}}/><Fl3 s={{width:110,top:10,right:-10}}/><Fl2 s={{width:130,bottom:20,left:10}}/><Fl1 s={{width:100,bottom:-10,right:10}}/>
    <div style={S.loginCard}>
      ...
      <div style={{textAlign:"left",maxWidth:340,margin:"0 auto"}}>
        <label style={{fontSize:14,fontWeight:700,color:"#5D4037",display:"block",marginBottom:6}}>커플 키</label>
        <input style={{...MI,fontSize:18,padding:"16px"}} placeholder="예: jm-sk-2027" value={key} onChange={e=>{setKey(e.target.value);setIsNew(false);setErr("");}} onKeyDown={e=>{if(e.key==="Enter")tryLogin();}}/>
        <p style={{fontSize:13,color:"#A1887F",margin:"10px 0 0",lineHeight:1.6}}>처음이면 원하는 키를 만드세요. 어떤 브라우저·기기에서든 같은 키로 들어오면 기록이 있어요.</p>
        {err&&<p style={{color:"#C62828",fontSize:14,margin:"8px 0 0"}}>{err}</p>}
        {isNew?(<div style={{marginTop:20,padding:20,background:"#FFF8E1",borderRadius:16,textAlign:"center"}}>
          <p style={{fontSize:16,color:"#5D4037",margin:"0 0 14px"}}>✨ <b>"{key}"</b>는 새로운 키예요!</p>
          <button style={{...S.btn1,width:"100%"}} onClick={createNew} disabled={busy}>{busy?"생성 중...":"새로 시작하기 🌷"}</button>
        </div>):(<button style={{...S.btn1,width:"100%",marginTop:20}} onClick={tryLogin} disabled={busy}>{busy?"확인 중...":"들어가기 →"}</button>)}
      </div>
    </div>
  </div>);
}
...
function SetupScreen({data,setData}){
  ...
  return(<div style={S.wrap}><Fl1 s={{width:130,top:-20,left:-20}}/><Fl3 s={{width:95,top:15,right:5}}/><Fl2 s={{width:110,bottom:30,left:15}}/>
    <div style={{...S.loginCard,maxWidth:480}}>
      <div style={{fontSize:48,marginBottom:6}}>💐</div>
      <h1 style={{fontSize:26,fontWeight:700,color:"#33691E",margin:"0 0 6px"}}>두 사람의 정보</h1>
      <p style={{fontSize:15,color:"#A1887F",margin:"0 0 28px",lineHeight:1.7}}>이름과 예정일을 입력해주세요</p>
      <div style={{display:"flex",flexDirection:"column",gap:18,textAlign:"left"}}>
        ...
        <div style={{display:"flex",gap:12}}>
          <div style={{flex:1,minWidth:0}}><label style={{fontSize:14,fontWeight:700,color:"#5D4037",display:"block",marginBottom:6}}>결혼 예정일</label><input type="date" style={MI} value={f.weddingDate} onChange={e=>setF({...f,weddingDate:e.target.value})}/></div>
          <div style={{flex:1,minWidth:0}}><label style={{fontSize:14,fontWeight:700,color:"#5D4037",display:"block",marginBottom:6}}>예식 시간</label><input type="time" style={MI} value={f.weddingTime} onChange={e=>setF({...f,weddingTime:e.target.value})}/></div>
        </div>
...
function Dash({data,setTab}){
  ...
  return(<div style={{display:"flex",flexDirection:"column",gap:20}}>
    <div style={S.dday}><Fl1 s={{width:160,top:-35,left:-25}}/><Fl3 s={{width:110,top:5,right:-10}}/><Fl2 s={{width:130,bottom:-20,right:40}}/><Fl1 s={{width:100,bottom:5,left:5}}/>
      <div style={{position:"relative",zIndex:1}}>
        <div style={{fontSize:13,fontWeight:600,color:"#7B6B5D",letterSpacing:3,marginBottom:10,opacity:0.5}}>OUR WEDDING</div>
        <div style={{fontSize:26,fontWeight:700,color:"#4E7D3A",marginBottom:14}}>{data.groomName} 🌿 ♥ 🌻 {data.brideName}</div>
        <div style={{fontSize:72,fontWeight:700,color:"#33691E",lineHeight:1}}>{dd>0?"D-"+dd:dd===0?"D-Day 🎉":"D+"+Math.abs(dd)}</div>
        <div style={{fontSize:17,color:"#689F38",marginTop:14}}>{fmtDate(data.weddingDate)} {data.weddingTime}</div>
      </div>
    </div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18}}>
      <div style={S.card}><div style={S.cl}>준비 진행률</div><div style={{height:12,background:"#F1F8E9",borderRadius:6,overflow:"hidden",marginBottom:14}}><div style={{height:"100%",background:"linear-gradient(90deg,#FDD835,#66BB6A)",borderRadius:6,width:pct+"%",transition:"width 0.5s"}}/></div><div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontSize:36,fontWeight:700,color:"#4E7D3A"}}>{pct}%</span><span style={{fontSize:16,color:"#8D6E63",alignSelf:"flex-end"}}>{dn}/{tot} 완료</span></div></div>
      <div style={S.card}><div style={S.cl}>예산 현황</div><div style={{fontSize:28,fontWeight:700,color:"#4E7D3A",marginBottom:6}}>{fmtWon(bT)}</div><div style={{fontSize:15,color:"#8D6E63"}}>지출 {fmtWon(bP)}</div><div style={{fontSize:15,color:"#2E7D32",fontWeight:600}}>잔여 {fmtWon(bT-bP)}</div></div>
    </div>
    ...
    <div style={S.card}>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><div style={S.cl}>{cur?(cur.emoji+" 지금 할 일 — "+cur.label):"모든 준비 완료!"}</div>{cur&&<button style={S.lnk} onClick={()=>setTab("checklist")}>전체 →</button>}</div>
      {cur&&<div style={{fontSize:15,color:"#8D6E63",marginBottom:16}}>📅 {pLabel(cur.period)} · {mBefore(cur.period,data.weddingDate)} · 기한 {deadln(cur.period)}</div>}
      {todo.slice(0,6).map(item=><div key={item.id} style={{display:"flex",alignItems:"center",gap:10,padding:"12px 16px",background:"#FAFAF5",borderRadius:14,marginBottom:8,flexWrap:"wrap"}}><Badge cat={item.cat}/><span style={{fontSize:16,color:"#3E2723"}}>{item.text}</span></div>)}
      {todo.length>6&&<div style={{fontSize:15,color:"#A1887F",textAlign:"center",marginTop:8}}>외 {todo.length-6}개</div>}
    </div>
  </div>);
}
...
function CLTab({data,setData}){
  ...
  return(<div style={S.tab}><h2 style={S.th}>🌿 체크리스트</h2>
      ...
      {data.checklist.map((ph,pi)=>{const st=pStat(ph.period),allD=ph.items.every(i=>i.done),dc=ph.items.filter(i=>i.done).length;return(
        <div key={pi} style={{...S.card,borderLeft:"4px solid "+(st==="current"?"#81C784":st==="past"?"#C8E6C9":"#E0E0E0")}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8,flexWrap:"wrap",gap:8}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}><span style={{width:28,height:28,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,fontWeight:700,color:"#fff",background:allD?"#66BB6A":st==="current"?"#FDD835":"#E0E0E0"}}>{allD?"✓":""}</span><span style={{fontSize:18,fontWeight:700,color:"#3E2723"}}>{ph.emoji} {ph.label}</span><span style={{fontSize:15,color:"#A1887F"}}>{dc}/{ph.items.length}</span></div>
            {st==="current"&&<span style={{padding:"5px 14px",borderRadius:8,fontSize:13,fontWeight:700,background:"#FFF8E1",color:"#F9A825"}}>지금</span>}
          </div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {ph.items.map(item=><div key={item.id} style={{display:"flex",gap:14,alignItems:"flex-start",padding:"12px 16px",borderRadius:14,cursor:"pointer",background:"#FAFAF5",opacity:item.done?0.4:1}} onClick={()=>toggle(pi,item.id)}>
              <div style={{width:26,height:26,borderRadius:8,border:"2px solid "+(item.done?"#66BB6A":"#D7CCC8"),background:item.done?"#66BB6A":"#fff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:15,fontWeight:700,color:"#fff",flexShrink:0}}>{item.done?"✓":""}</div>
              <div style={{flex:1}}><div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}><Badge cat={item.cat}/><span style={{textDecoration:item.done?"line-through":"none",fontSize:16,color:"#3E2723"}}>{item.text}</span></div>{item.note&&<div style={{fontSize:14,color:"#A1887F",marginTop:5}}>💡 {item.note}</div>}</div>
            </div>)}
          </div>
        </div>);})}
    </div>
...
function BudTab({data,setData}){
  ...
  const sEdit=item=>{setEd(item.id);setEf({name:item.name,category:item.category,unitPrice:String(item.unitPrice),qty:String(item.qty),paid:String(item.paid)});};
  const svEdit=id=>{const u=parseInt(ef.unitPrice)||0,q=parseInt(ef.qty)||1;setData({...data,budget:data.budget.map(b=>b.id===id?{...b,name:ef.name,category:ef.category,unitPrice:u,qty:q,amount:u*q,paid:parseInt(ef.paid)||0}:b)});setEd(null);};
  ...
  const calcAmt=(parseInt(f.unitPrice)||0)*(parseInt(f.qty)||1);
  const editAmt=(parseInt(ef.unitPrice)||0)*(parseInt(ef.qty)||1);

  return(<div style={S.tab}><h2 style={S.th}>🌻 예산 관리</h2>
    ...
    <button style={{...S.btn1,width:"100%",marginBottom:22}} onClick={()=>setShow(true)}>+ 예산 항목 추가</button>

    <Modal title="예산 항목 추가" open={show} onClose={()=>setShow(false)}>
      <ModalField label="항목명"><input style={MI} placeholder="예: 웨딩홀 대관료" value={f.name} onChange={e=>setF({...f,name:e.target.value})}/></ModalField>
      <ModalField label="카테고리"><select style={MI} value={f.category} onChange={e=>setF({...f,category:e.target.value})}>{Object.entries(CAT).map(([k,v])=><option key={k} value={k}>{v.n}</option>)}</select></ModalField>
      <div style={{display:"flex",gap:12,marginBottom:18}}>
        <div style={{flex:1}}><label style={{fontSize:14,fontWeight:700,color:"#5D4037",display:"block",marginBottom:6}}>단가</label><input style={MI} type="number" placeholder="0" value={f.unitPrice} onChange={e=>setF({...f,unitPrice:e.target.value})}/></div>
        <div style={{width:30,display:"flex",alignItems:"flex-end",justifyContent:"center",paddingBottom:14,fontSize:20,color:"#A1887F"}}>×</div>
        <div style={{flex:1}}><label style={{fontSize:14,fontWeight:700,color:"#5D4037",display:"block",marginBottom:6}}>수량</label><input style={MI} type="number" placeholder="1" value={f.qty} onChange={e=>setF({...f,qty:e.target.value})}/></div>
      </div>
      <div style={{background:"#F1F8E9",borderRadius:14,padding:"16px 20px",marginBottom:18,textAlign:"center"}}>
        <div style={{fontSize:14,color:"#689F38",fontWeight:600,marginBottom:4}}>견적 금액</div>
        <div style={{fontSize:28,fontWeight:700,color:"#33691E"}}>{fmtWon(calcAmt)}</div>
      </div>
      <ModalField label="지출액 (이미 지급한 금액)"><input style={MI} type="number" placeholder="0" value={f.paid} onChange={e=>setF({...f,paid:e.target.value})}/></ModalField>
      <button style={{...S.btn1,width:"100%"}} onClick={add}>추가하기</button>
    </Modal>

    <Modal title="예산 항목 수정" open={ed!==null} onClose={()=>setEd(null)}>
      <ModalField label="항목명"><input style={MI} value={ef.name||""} onChange={e=>setEf({...ef,name:e.target.value})}/></ModalField>
      <ModalField label="카테고리"><select style={MI} value={ef.category||"venue"} onChange={e=>setEf({...ef,category:e.target.value})}>{Object.entries(CAT).map(([k,v])=><option key={k} value={k}>{v.n}</option>)}</select></ModalField>
      <div style={{display:"flex",gap:12,marginBottom:18}}>
        <div style={{flex:1}}><label style={{fontSize:14,fontWeight:700,color:"#5D4037",display:"block",marginBottom:6}}>단가</label><input style={MI} type="number" value={ef.unitPrice||""} onChange={e=>setEf({...ef,unitPrice:e.target.value})}/></div>
        <div style={{width:30,display:"flex",alignItems:"flex-end",justifyContent:"center",paddingBottom:14,fontSize:20,color:"#A1887F"}}>×</div>
        <div style={{flex:1}}><label style={{fontSize:14,fontWeight:700,color:"#5D4037",display:"block",marginBottom:6}}>수량</label><input style={MI} type="number" value={ef.qty||""} onChange={e=>setEf({...ef,qty:e.target.value})}/></div>
      </div>
      <div style={{background:"#F1F8E9",borderRadius:14,padding:"16px 20px",marginBottom:18,textAlign:"center"}}>
        <div style={{fontSize:14,color:"#689F38",fontWeight:600,marginBottom:4}}>견적 금액</div>
        <div style={{fontSize:28,fontWeight:700,color:"#33691E"}}>{fmtWon(editAmt)}</div>
      </div>
      <ModalField label="지출액 (이미 지급한 금액)"><input style={MI} type="number" value={ef.paid||""} onChange={e=>setEf({...ef,paid:e.target.value})}/></ModalField>
      <button style={{...S.btn1,width:"100%"}} onClick={()=>svEdit(ed)}>저장하기</button>
    </Modal>
    ...
    {data.budget.map(item=>{const pp=item.amount>0?Math.round(item.paid/item.amount*100):0;return(
      <div key={item.id} style={{...S.card,marginBottom:12}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}><Badge cat={item.category}/><span style={{fontWeight:700,fontSize:17,color:"#3E2723"}}>{item.name}</span></div>
          <div style={{display:"flex",gap:8}}><button style={S.ebtn} onClick={()=>sEdit(item)}>수정</button><button style={S.dbtn} onClick={()=>rm(item.id)}>✕</button></div>
        </div>
...
function DatesTab({data,setData}){
  ...
  return(<div style={S.tab}><h2 style={S.th}>📌 주요 일정</h2>
    <button style={{...S.btn1,width:"100%",marginBottom:22}} onClick={()=>setShow(true)}>+ 일정 추가</button>
    <Modal title="주요 일정 추가" open={show} onClose={()=>setShow(false)}>
      <ModalField label="일정명"><input style={MI} placeholder="예: 상견례, 스튜디오 촬영" value={f.name} onChange={e=>setF({...f,name:e.target.value})}/></ModalField>
      <ModalField label="날짜"><input type="date" style={MI} value={f.date} onChange={e=>setF({...f,date:e.target.value})}/></ModalField>
      <ModalField label="메모 (선택)"><input style={MI} placeholder="장소, 시간 등" value={f.memo} onChange={e=>setF({...f,memo:e.target.value})}/></ModalField>
      <button style={{...S.btn1,width:"100%"}} onClick={add}>추가하기</button>
    </Modal>
    ...
    {sorted.map(d=>{const dd=getDday(d.date);const past=new Date(d.date)<today;return<div key={d.id} style={{...S.card,marginBottom:12,opacity:past?0.5:1,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      <div><div style={{fontSize:18,fontWeight:700,color:"#3E2723"}}>{d.name}</div><div style={{fontSize:15,color:"#689F38",marginTop:4}}>{fmtDate(d.date)} {dd>0?"(D-"+dd+")":dd===0?"(오늘!)":"(지남)"}</div>{d.memo&&<div style={{fontSize:14,color:"#A1887F",marginTop:4}}>📝 {d.memo}</div>}</div>
      <button style={S.dbtn} onClick={()=>rm(d.id)}>✕</button>
    </div>;})}
  </div>);
}
...
function VenTab({data,setData}){
  ...
  return(<div style={S.tab}><h2 style={S.th}>📇 업체 연락처</h2>
    <button style={{...S.btn1,width:"100%",marginBottom:22}} onClick={()=>setShow(true)}>+ 업체 추가</button>
    <Modal title="업체 추가" open={show} onClose={()=>setShow(false)}>
      <ModalField label="업체명"><input style={MI} placeholder="예: 보네르스포사" value={f.name} onChange={e=>setF({...f,name:e.target.value})}/></ModalField>
      <ModalField label="카테고리"><select style={MI} value={f.category} onChange={e=>setF({...f,category:e.target.value})}>{Object.entries(CAT).map(([k,v])=><option key={k} value={k}>{v.n}</option>)}</select></ModalField>
      <ModalField label="연락처"><input style={MI} placeholder="010-0000-0000" value={f.phone} onChange={e=>setF({...f,phone:e.target.value})}/></ModalField>
      <ModalField label="메모 (선택)"><input style={MI} placeholder="담당자, 특이사항 등" value={f.memo} onChange={e=>setF({...f,memo:e.target.value})}/></ModalField>
      <button style={{...S.btn1,width:"100%"}} onClick={add}>추가하기</button>
    </Modal>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:14}}>
      {data.vendors.length===0&&<div style={S.empty}>업체 정보를 추가해보세요 🌻</div>}
      {data.vendors.map(v=><div key={v.id} style={S.card}><div style={{display:"flex",justifyContent:"space-between",marginBottom:10}}><Badge cat={v.category}/><button style={S.dbtn} onClick={()=>rm(v.id)}>✕</button></div><div style={{fontWeight:700,fontSize:18,color:"#3E2723",marginBottom:6}}>{v.name}</div>{v.phone&&<div style={{fontSize:16,color:"#558B2F"}}>📞 {v.phone}</div>}{v.memo&&<div style={{fontSize:15,color:"#8D6E63",marginTop:6}}>📝 {v.memo}</div>}</div>)}
    </div>
  </div>);
}

/* ═══ SETTINGS ═══ */
function SetTab({data,setData,coupleKey,onLogout}){
  ...
  return(<div style={S.tab}><h2 style={S.th}>⚙️ 설정</h2>
    <div style={S.card}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:16,maxWidth:480}}>
        ...
        <div style={{display:"flex",gap:12,marginTop:8}}><button style={S.btn1} onClick={save}>{saved?"✓ 저장!":"저장"}</button><button style={{padding:"14px 22px",background:"#EFEBE9",color:"#8D6E63",border:"none",borderRadius:14,fontSize:15,fontWeight:600,cursor:"pointer",fontFamily:FONT}} onClick={onLogout}>로그아웃</button></div>
      </div>
    </div>
  </div>);
}
...
export default function App(){
  ...
  const saveRef=useState({timer:null})[0];
  const setData=useCallback(nd=>{setDR(nd);if(saveRef.timer)clearTimeout(saveRef.timer);saveRef.timer=setTimeout(()=>{if(ck){setSaving(true);dbSave(ck,nd).then(()=>setSaving(false));}},800);},[ck]);
  ...
  return(<div style={S.app}>
    <nav style={S.side}>
      ...
      <div style={{fontSize:13,color:"#8D6E63",padding:"0 14px 14px",borderBottom:"1px solid #E8E0D8",marginBottom:10}}>{data.groomName} ♥ {data.brideName}{saving&&<span style={{fontSize:11,color:"#A5D6A7",marginLeft:6}}>☁️</span>}</div>
    ...
    <main style={S.main}>
      ...
      {tab==="settings"&&<SetTab data={data} setData={setData} coupleKey={ck} onLogout={onLogout}/>}
    </main>
  </div>);
}
